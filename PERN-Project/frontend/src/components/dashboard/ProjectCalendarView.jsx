      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b border-slate-100">
          {DAYS.map((d) => (
            <div
              key={d}
              className="py-2 text-center text-[10px] font-bold text-slate-400 uppercase tracking-wider"
            >
              {d}
            </div>
          ))}
                ...
                {day && (
                  <>
                    ...
                    <div className="space-y-0.5">
                      {visible.map((issue) => {
                        const sCfg =
                          statusColors[issue.status] || statusColors.TODO;
                        return (
                          <div
                            key={issue.id}
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full shrink-0 ${priorityDot[issue.priority]}`}
                            />
                              ...
                              <div className="absolute left-0 top-full mt-1 z-30 w-52 bg-white border border-slate-200 rounded-xl shadow-xl p-3 pointer-events-none">
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span
                                      className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${sCfg.pill}`}
                                    >
            ...
            {monthIssues.map((issue) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      className={`text-[10px] font-bold uppercase ${isOverdue ? "text-red-400" : isDueToday ? "text-yellow-600" : "text-slate-400"}`}
                    >
                      {formatDate(issue.dueDate).slice(3, 8)}
                    </span>
                    <span
                      className={`text-sm font-extrabold leading-none ${isOverdue ? "text-red-600" : isDueToday ? "text-yellow-700" : "text-slate-700"}`}
                    >
                      {due.getDate()}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      />
                      <p
                        className={`text-sm font-semibold truncate ${issue.status === "DONE" ? "line-through text-slate-400" : "text-slate-800"}`}
                      >
                        {issue.title}
                      </p>
                    </div>
                  ...
                  {/* Status */}
                  <span
                    className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-lg border ${sCfg.pill}`}
                  >
                    {issue.status.replace("_", " ")}
                  </span>
      ...
      {monthIssues.length === 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-12 text-center">
          ...
          <p className="text-sm font-bold text-slate-400">
            No tasks due in {MONTHS[current.month]}
          </p>
          <p className="text-xs text-slate-300 mt-1">
            Set due dates on issues to see them here
          </p>
        </div>
      )}
    </div>
