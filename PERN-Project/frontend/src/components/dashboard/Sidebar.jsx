...
import { 
  BarChart3, FolderOpen, CheckCircle, Zap, 
  Flag, Users, LogOut, Shield 
} from 'lucide-react';
    ...
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 z-30 shadow-[0_0_15px_rgba(0,0,0,0.02)]">
        ...
        {selectedProject && currentView.startsWith("PROJECT_") ? (
          ...
          <nav className="p-4 space-y-1">
            ...
            <button onClick={() => {
              ...
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${currentView === "GLOBAL_ISSUES" ? "bg-blue-200 text-blue-800" : "bg-slate-100 text-slate-500"}`}>
                {myIssuesCount}
              </span>
            </button>
          </nav>
      ...
      <div className="p-4 border-t border-slate-100 bg-slate-50/50">
        <div className="flex items-center gap-3 mb-4 px-1">
           <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-extrabold shadow-sm border border-white bg-linear-to-br from-blue-500 to-indigo-600 text-white relative`}>
             {initials}
             <div className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-white ${role.dot}`} />
           </div>
           <div className="flex-1 overflow-hidden">
              <p className="text-sm font-bold text-slate-900 truncate">{currentUser.name || "User"}</p>
              <p className="text-[10px] tracking-wider uppercase font-bold text-slate-500 truncate">{roleLabel}</p>
           </div>
        </div>
        <button
          onClick={onLogout}
